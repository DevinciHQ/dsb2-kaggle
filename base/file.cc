...
UniqueFILE OpenFileStream(const char* path, const char* mode) {
  auto result = UniqueFILE(fopen(path, mode), fclose);
  if (!result) {
...
kj::Array<const char> ReadFD(int fd) {
  ...
  if (size == -1) {
    ...
    return kj::Array<const char>(buffer.release(), size, FreeArrayDisposer::instance);
  } else if (size == 0) {
...
kj::Array<const char> ReadFile(const char* path) {
